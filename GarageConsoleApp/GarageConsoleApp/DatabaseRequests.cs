...
public static class DatabaseRequests
{
    ...
    public static void AddCarsQuery(int idType, string title, string stateNumber, int numberPassengers)
    {
        var querySql = $"INSERT INTO car(id_type_car, name, state_number, number_passengers) VALUES ({idType}, '{title}', '{stateNumber}', {numberPassengers})";
        using var cmd = new NpgsqlCommand(querySql, DatabaseService.GetSqlConnection());
        cmd.ExecuteNonQuery();
    }
    ...
    public static void GetCarQuery()
    {
        ...
        while (reader.Read())
        {
            Console.WriteLine($"Тип: {reader[0]} Название: {reader[1]} модель: {reader[2]} номер: {reader[3]} кол во пссажиров: {reader[4]}");
        }
    }

    public static void AddItineraryQuery(string title)
    {
        var querySql = $"INSERT INTO itinerary(name) VALUES ('{title}')";
        using var cmd = new NpgsqlCommand(querySql, DatabaseService.GetSqlConnection());
        cmd.ExecuteNonQuery();
    }
    ...
    public static void GetRoutQuery()
    {
        ...
        while (reader.Read())
        {
            Console.WriteLine($"Id водителя: {reader[0]} id машины: {reader[1]} id маршрута: {reader[2]} количество пассажиров: {reader[3]}");
        }
    }
}